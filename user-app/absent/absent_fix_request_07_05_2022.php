...
//	$abs_sql = "select * from `absent_fix_entries` where `emp_code`='".$uniquecode."' and place!='Week off' and `for_date` like '%".date('Y-m')."%' ";
	...
	if( $result_check_sql->num_rows == 0 ){
		...
		//	$update_user_check_sqls= $mysqli->query("update `user` set can_mark_attendance ='yes' where `emp_code`='".$uniquecode."' ");
			
		$tch = $mysqli->query("update user set can_mark_attendance = 'yes' where emp_code = '$uniquecode' ");
