...
if( $user->num_rows > 0 ) {
	...
	$sql = "select * from `attendance` where emp_code='".$uniquecode."' and `date`>='".$date."' and `date`<='".$date1."' and `halfs`='Absent' order by `date` desc ";
	$result = $mysqli->query($sql);
	if( $result->num_rows > 0 ){
		while( $row = $result->fetch_assoc() ){
		    
		 /*   $sql2 = "select * from `absent_fix_entries` where emp_code='".$uniquecode."' and `for_date`='".$row['date']."' ";
	...
	$sql1 = "select * from `absent_fix_entries` where `emp_code`='".$uniquecode."' and `for_date`>='".$date."' and `for_date`<='".$date1."' order by `request_date` desc ";
	...
	$data = array('list'=>$list, 'request'=>$request);
