...
//$date_att = explode('/', $last_attendance_date);
//$attendance_date = $date_att[2] . "-" . $date_att[1] . "-" . $date_att[0];
...
if( $user->num_rows > 0 ) {
  ...
  //   echo json_encode(array('status'=>'0', 'data'=>'', 'message'=>'No.. This feature is disabled.'));
	...
	if( $result_auto_sql->num_rows > 4 ){
	    echo json_encode(array('status'=>'0', 'message'=>'Limit exhausted for Auto Out, Contact Manager/HR.'));
		exit();
	}
	...
	if( $result->num_rows > 0 ){
		...
		if( strtotime($attendance_data['in_time']) >= strtotime($selected_out_time) ) {
			echo json_encode(array('status'=>'0', 'message'=>'Out time should be greater than In Time.'));
			exit();
		}
	} 
	/* else {
	...
	*/
	...
	if( $result_check_sql->num_rows == 0 ){
		...
		if( $auto_out_insert > 0 ){
		 ...
		 $update_user_sql_old2 = "update `user` set can_mark_attendance ='yes' where `emp_code`='".$uniquecode."' ";	
		 $update_user_check_sql_old = $mysqli->query($update_user_sql_old2);
